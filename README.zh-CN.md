...
agent 和 TUI 使用相同的作用域解析：优先项目板，其次全局板。工具也接受 `scope: project | global | auto`；两种板都不存在时，agent 不会擅自创建，而是先询问用户选择。完成状态使用显式的 `set_done`，避免 agent 重试时把状态意外反转；卡片或列名有歧义时，工具会拒绝猜测并要求先读取看板。

## TUI 能做什么

- 多列、自适应宽度与高度的嵌入式面板；少量卡片时自动收紧，内容较多时最高约 34 行并在列内滚动。
...
- 每张卡片在看板中最多显示两行：第一行标题，第二行显示首段正文、时间和标签；仍有内容时以 `…` 提示。
- 进入详情后按 `y`，一键复制标题、时间、标签和完整正文。
...
旧看板只是迁移来源，不是运行依赖。要把已有的 Markdown 看板复制到当前 Pi 项目：

```text
/kanban import "D:\path\to\legacy-board.md"
```

扩展会先验证来源，再复制到当前项目的 `.pi/kanban.md`；不会修改源文件。若项目看板已经存在，Pi 会先要求确认。导入完成后的主数据是项目内文件，此后只需要 `/kanban`。如需全局板，可先用 `/kanban global` 创建，然后在 Pi 中添加卡片。
